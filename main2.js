...
// â†’â†’â†’ Function Definitions â†â†â†
	...
	var switchTiles = function(a,b) {
		...
		var holder = {};
		holder.tileX = b.tileX;		holder.tileY = b.tileY; //[TILES]: Switch tilesâ€¦
			...
			if(!matches.length) {
				var drawRating = function() {
					if(matchesMadeThisMove > 4) {
						drawBigOverlay('Delicious!', Width/5, Height/6, 50, ["#F8DB63", "#CF8A09"]);
					} else if(matchesMadeThisMove > 3) {
						drawBigOverlay('Divine!',    Width/5, Height/6, 50, ["#F8DB63", "#CF8A09"]);
					} else if(matchesMadeThisMove > 2) {
						drawBigOverlay('Sweet!',     Width/5, Height/6, 50, ["#F8DB63", "#CF8A09"]);
					} else if(matchesMadeThisMove > 1) {
						drawBigOverlay('Tasty!',     Width/5, Height/6, 50, ["#F8DB63", "#CF8A09"]);
					}
				};
		...
		var matchesFound = _
			.flatten(
				makeField().map(function(column, x) {
					return column.map(function(row, y) { //We scan for intersection bonuses first because we wouldn't want to miss one because a line bonus had already spoken for it. (Only when the first tile was â‡± would the intersection be counted.)
	...
	sugarRush = function() {
		...
		if(!!_.head(bonusTiles)) {
			drawBigOverlay('Sugar Rush', Width/5, Height/6, 40, ["#F8DB63", "#CF8A09"]);
...
// â†’â†’â†’ Initial Setup â†â†â†
	...
	var mode = typeof iMode !== 'undefined' && iMode || 'time'; //turns, time
	...
	var remainingTiles = watchableCounter(
		typeof iMoves !== 'undefined' && iMoves || 35);
	var remainingTime = watchableCounter(
		typeof iTime !== 'undefined' && iTime || 180);
	
	var jelly = typeof iJelly !== 'undefined' && !!iJelly || true;
